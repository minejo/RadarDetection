...
function [track_normal, ones_result1] = maintain_formaltrack(track_normal, ones_result, TT, t)
...
dis_diff = 4; %目标与最近轨迹目标相差的距离不能超过dis_diff m
v_diff =4; %目标速度与预计速度相差不能超过v_diff
track_normal1 = track_normal; %由于track_normal处于一级循环中，集合变化导致参数i遍历失败，因而操作副本
ones_result1 = ones_result;%副本
k=0;%删除副本的序号
for i=1:formal_num
    ...
    for j = 1:scan_num
        if(~isempty(ones_result1))
            ...
            [isTrue, Weightmean] = formaltrackfindobject(oneformaltrack, one, time_diff,time_diff_min, dis_diff, v_diff, fangwei_diff,TT,t);%该点是否多个条件都符合，如果符合权重加和为多少
    ...
    if(minWeight ~= 10000) %确定找到合适的目标
        formal = [oneformaltrack; ones_result(scan_result,:)];
        track_normal{i} = formal;%维持正式轨迹并将结果修改到集合中
        ones_result1(j-k,:) = []; %删除已处理的目标
        k=k+1;
    end
