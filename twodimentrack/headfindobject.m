...
function [isTrue, Weightmean]= headfindobject(head, one,time_diff,time_diff_min,dis_diff,v_diff,fangwei_diff)
...
isTrue = 0;
Weightmean = 1;
if( abs(distance-one_distance) <= dis_diff && abs(velocity - one_velocity) <= v_diff && abs(fangwei - one_fangwei) <= fangwei_diff && abs(time-one_time) <= time_diff && abs(time-one_time) >= time_diff_min)
  isTrue = 1;
