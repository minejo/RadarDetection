...
A_init_l = []; %目标的初始横向加速度
A_init_w = []; %目标的初始纵向加速度
maxIndexL = fix(0.8*map_length / map_l); %随机取的目标的横坐标的最大范围，避免取到边界的极端情况
minIndexL = fix(0.2*map_length / map_l);
maxIndexW = fix(0.8*map_width / map_w);
minIndexW = fix(0.2*map_width / map_w);
