...
function [ hasObject, choice ] = vicfar(refer,value, Pfa)
...
VI_after=1;
hasObject=0; %是否检测出目标
avg_x_before=mean(refer(1:N/2));
avg_x_after=mean(refer(N/2+1:N));
for i=1:N/2
   VI_before=VI_before+1/(N/2-1)*(refer(i)-avg_x_before)^2/avg_x_before^2;
   VI_after=VI_after+1/(N/2-1)*(refer(i+N/2)-avg_x_after)^2/avg_x_after^2; 
end
